...
for config_file in "${CONFIG_DIR}"/grounding_dino_swin-b_finetune_*.py; do
    if [ -f "${config_file}" ] && [[ "$(basename "${config_file}")" == *"_1shot.py" ]]; then
        ...
        ./tools/dist_train.sh "${config_file}" 1 9999 "4" --work-dir "${work_dir}"
        ...
        ./tools/dist_test.sh "${config_file}" "${ckpt_path}" 1 9999 "4" --work-dir "${work_dir_test}" --out "${work_dir_test}/${dataset_name}.pkl"
